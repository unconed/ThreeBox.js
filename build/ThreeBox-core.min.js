...
!0;a.lastHover=a.origin={x:b.pageX,y:b.pageY};b.preventDefault()};this._mouseUp=function(){a.drag=!1};this._mouseMove=function(b){if(a.drag){var c={x:b.pageX-a.origin.x,y:b.pageY-a.origin.y},d={x:b.pageX-a.lastHover.x,y:b.pageY-a.lastHover.y};a.lastHover={x:b.pageX,y:b.pageY};a.moved(a.origin,c,d)}};this._touchstart=function(b){a.width=a.element&&a.element.offsetWidth;a.height=a.element&&a.element.offsetHeight;a.drag=!0;a.lastHover=a.origin={x:b.touches[0].pageX,y:b.touches[0].pageY};b.preventDefault()};
this._touchmove=function(b){if(a.drag){var c={x:b.touches[0].pageX-a.origin.x,y:b.touches[0].pageY-a.origin.y},d={x:b.touches[0].pageX-a.lastHover.x,y:b.touches[0].pageY-a.lastHover.y};a.lastHover={x:b.touches[0].pageX,y:b.touches[0].pageY};a.moved(a.origin,c,d)}b.preventDefault();b.stopPropagation()};this._touchend=function(){a.drag=!1};this.element&&(this.element.addEventListener("mousedown",this._mouseDown,!1),document.addEventListener("mouseup",this._mouseUp,!1),document.addEventListener("mousemove",
...
this._touchend),this.element.removeEventListener("touchmove",this._touchmove))},moved:function(a,b,c){this.phi+=c.x*this.speed/this.width;this.theta=Math.min(\u03c0/2,Math.max(-\u03c0/2,this.theta+c.y*this.speed/this.height));this.emit("change")},update:function(){this.camera.position.x=Math.cos(this.phi)*Math.cos(this.theta)*this.orbit;this.camera.position.y=Math.sin(this.theta)*this.orbit;this.camera.position.z=Math.sin(this.phi)*Math.cos(this.theta)*this.orbit;this.camera.position.addSelf?this.camera.position.addSelf(this.lookAt):
this.camera.position.add(this.lookAt);this.camera.lookAt(this.lookAt)}};ThreeBox.OrbitControls.bind=function(a,b,c){return new ThreeBox.OrbitControls(a,b,c)};MicroEvent.mixin(ThreeBox.OrbitControls);
