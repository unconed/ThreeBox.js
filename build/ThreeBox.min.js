...
ThreeBox.ElementResize=function(c,d,f,i){this.scale=i.scale||1;this.orbit=i.orbit;i=this.callback=function(){var j=Math.floor(f.offsetWidth),i=Math.floor(f.offsetHeight);c.domElement.style.position="absolute";c.domElement.style.width=j+"px";c.domElement.style.height=i+"px";var k=Math.floor(j/this.scale),s=Math.floor(i/this.scale);c.setSize(k,s);d.aspect=j/i;d.top=this.orbit/2;d.bottom=-d.top;d.left=-d.top*d.aspect;d.right=-d.bottom*d.aspect;d.updateProjectionMatrix();this.emit("resize",k,s)}.bind(this);
window.addEventListener("resize",i,!1);f.addEventListener("resize",i,!1);setTimeout(i,0)};ThreeBox.ElementResize.bind=function(c,d,f,i){return new ThreeBox.ElementResize(c,d,f,i)};ThreeBox.ElementResize.prototype.scale=function(c){this.scale=c};ThreeBox.ElementResize.prototype.unbind=function(){window.removeEventListener("resize",callback);domElement.removeEventListener("resize",callback)};MicroEvent.mixin(ThreeBox.ElementResize);
